...
def scan_ball(mode):
	...
	if pan_step > 0:		
		pan_step = rospy.get_param("/united_soccer_params/player/Pan_Step")
	else:				
		pan_step = rospy.get_param("/united_soccer_params/player/Pan_Step") * -1
	if tilt_step > 0:	
		tilt_step = rospy.get_param("/united_soccer_params/player/Tilt_Step") 
	else:				
		tilt_step = rospy.get_param("/united_soccer_params/player/Tilt_Step") * -1
...
def head_track_ball():
	...
	global freq
	dt = 1.0 / float(freq)
	KP_pan = rospy.get_param("/united_soccer_params/player/Pan_KP")
	KI_pan = rospy.get_param("/united_soccer_params/player/Pan_KI")
	KD_pan = rospy.get_param("/united_soccer_params/player/Pan_KD")
	KP_tilt = rospy.get_param("/united_soccer_params/player/Tilt_KP")
	KI_tilt = rospy.get_param("/united_soccer_params/player/Tilt_KI")
	KD_tilt = rospy.get_param("/united_soccer_params/player/Tilt_KD")
	if ball_x != -1 and ball_y != -1:	
...
def body_track_ball():
	global pos_pan, pos_tilt
	KP_body = rospy.get_param("/united_soccer_params/player/Body_KP")
	if ball_x != -1 and ball_y != -1:	 
...
def ball_positioning(setPoint_X, setPoint_Y, speed=0.10):
	...
	KP_ball_positioning_y = rospy.get_param("/united_soccer_params/player/KP_Ball_Pos_Y")
...
def ball_positioning2(setPoint_X, setPoint_Y, speed=0.10):
	...
	KP_ball_positioning_x = rospy.get_param("/united_soccer_params/player/KP_Ball_Pos_X")
	KP_ball_positioning_y = rospy.get_param("/united_soccer_params/player/KP_Ball_Pos_Y")
...
def kick(): 
	...
	pPan_kick = rospy.get_param("/united_soccer_params/player/Pan_Kick")
	pTilt_kick = rospy.get_param("/united_soccer_params/player/Tilt_Kick")
		...
		if play :
			...
			if state == "initial":
				...
				if ball_lost(20):
					...
					head_track_ball()
					followBall(0)
					if pos_tilt >= -0.3 and ball_x != -1 and ball_y != -1:
						state = "positioning"
					# if button[1] == 1:
						# state = "kick"
				...
				if ball_lost(20):
					...
					head_track_ball()
					set_point_x = rospy.get_param("/united_soccer_params/player/Pan_Kick")
					set_point_y = rospy.get_param("/united_soccer_params/player/Tilt_Kick")
					# if pos_pan > 0:
					ball_positioning(-set_point_x, set_point_y)
				...
				if ball_lost(20):
					...
					if count_goal_found > 5:
						...
						rotate_alpha = pos_pan * rospy.get_param("/united_soccer_params/player/KP_Compass_A")
						rotate_y = 0.3
						if pos_tilt > -0.3:
							walk(0.00, round(rotate_y, 3), round(rotate_alpha,3))
						else:
							error_tilt = pos_tilt - rospy.get_param("/united_soccer_params/player/Tilt_Kick")
							rotate_x = -error_tilt * rospy.get_param("/united_soccer_params/player/KP_Compass_X")
