...
def scan_line(mode):
    ...
    if pan_step > 0:        
        pan_step = rospy.get_param("/marathon_params/player/Pan_Step")
    else:               
        pan_step = rospy.get_param("/marathon_params/player/Pan_Step") * -1
    if tilt_step > 0:   
        tilt_step = rospy.get_param("/marathon_params/player/Tilt_Step") 
    else:               
        tilt_step = rospy.get_param("/marathon_params/player/Tilt_Step") * -1
    ...
    if mode == 0: # normal
        ...
        pos_pan += pan_step
        pos_tilt = rospy.get_param("/marathon_params/player/Tilt_Angle")
        if pos_pan >= pan_max or pos_pan <= pan_min:
            pan_step *= -1  
...
def head_track_line():
    ...
    if line_centre_x != -1 and line_centre_y != -1:
        ...
        # print("Marker : %s Line Centre : %d,%d Line Angle: %d", marker, line_centre_x, line_centre_y, line_angle)
        pos_pan, pos_tilt = head_pos.data
        head_pub.publish(head_pos)

def body_track_line():
    global pos_pan, pos_tilt
    KP_body = rospy.get_param("/marathon_params/player/Body_KP")
    if line_centre_x != -1:   
...
def main():
        ...
        if play :
            ...
            if state == "initial":
                ...
                if line_lost(20):
                    ...
                    walk(0.04, 0.0, shift)
                # PID body
                # if marker.data == "forward":
                #     state = "go_ahead"

                # if marker.data == "right":
                #     state = "turn_right"
                ...
                if line_centre_x != -1 and line_centre_y != -1:
                    ...
                    count_line = 0
                if count_line > line_threshold:
                    count_line = 0
                    state = "follow_line"

            elif state == "turn_left":
                # rotate compass
                if count_left > left_threshold:
                    state = "go_ahead"
                    count_left = 0
                else:
                    walk(0.00, 0.00, 0.3)
    ...
    # rate = rospy.spin()
    print("Marathon Player - Shut Down")
